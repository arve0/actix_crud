 ...
 *
 * 1. Default sqlite is twice as slow as when using write-ahead logging.
...
fn sqlite_benchmark(c: &mut Criterion) {
  ...
  c.bench_function("DEFAULT - unprepared statement", |b| {
    let db = create_filled_db(NUMBER_OF_ROWS);

    b.iter(|| {
      let value: u32 = db.query_row("SELECT value FROM bench WHERE id = ?1", &[123], |r| r.get(0)).unwrap();
    })
  });
  ...
  c.bench_function("DEFAULT - get same row", |b| {
    ...
    b.iter(|| run_query(&mut get_value, || { get_random_id(); 5431 }))
  });
  ...
  c.bench_function("DEFAULT - get random row", |b| {
    ...
    b.iter(|| run_query(&mut get_value, get_random_id))
  });
...
fn fill_db(db: &mut Connection, number_of_rows: u32) {
  ...
  { // drop `insert` before commiting
    let mut insert = transaction.prepare("INSERT INTO bench (id, value) values (?1, ?2)").unwrap();
...
fn enable_write_ahead_logging(db: &Connection) {
    ...
    let result: String = db.pragma_update_and_check(None, "journal_mode", &"wal", |row| row.get(0)).unwrap();
    assert!("wal" == &result);
}

fn run_query<F>(statement: &mut Statement, get_id: F) -> u32
where
  F: Fn() -> u32,
{
  let id = get_id();
...
fn get_random_id() -> u32 {
  SmallRng::from_seed([1,2,3,4,5,6,7,8,9,0,2,3,4,5,6,7]).gen_range(0, NUMBER_OF_ROWS)
}
